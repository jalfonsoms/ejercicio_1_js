const consumeApiWithAxios = async (url) => {
    try{
        ...
        console.log(`La peticion a la api se completó correctamentne con status: ${response.status}`);
        return await response.data;
    } catch (error){
        console.error(`Falló la peticion a la api con error: ${error.message}`);
    }
}
